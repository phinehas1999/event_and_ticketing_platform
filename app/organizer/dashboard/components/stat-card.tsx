...
export function StatCard({ title, value, icon, trend, alert }: StatCardProps) {
  return (
    <div className={`glass-card p-6 rounded-2xl relative overflow-hidden group ${alert ? 'border-amber-500/30' : ''}`}>
      <div className="relative z-10 flex justify-between items-start">
        <div>
          <p className="text-slate-400 text-sm font-medium mb-1">{title}</p>
          <h3 className="text-3xl font-bold text-slate-100">{value}</h3>
          {trend && <p className="text-emerald-400 text-xs mt-2 font-medium">{trend}</p>}
        </div>
